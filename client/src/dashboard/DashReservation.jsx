...
import img from "../image/profil.jpg";
export default function DashReservation() {
  const [reservation, setreservation] = useState([]);
  const fetchreservation = async () => {
    try {
      const response = await axios.get(`http://localhost:3001/allreservation`);
    ...
    try {
      ...
      if (confirmation) {
        const response = await axios.delete(
          `http://localhost:3001/deletereservation/${id}`
        );
  ...
  return (
    <div className={dash.bod}>
      ...
      <div>
        <h1 className={dash.h1}>Liste des Reservation</h1>
        <table className={dash.table}>
          <thead>
            <tr>
              <th className={dash.th1}></th>
              <th className={dash.th1}>Fullname</th>
              <th className={dash.th1}>Number</th>
              <th className={dash.th1}>Cin</th>
              <th className={dash.th1}>Email</th>
              <th className={dash.th1}>Event</th>
              <th className={dash.th1}>NumberPlaces</th>
              <th className={dash.th1}></th>
            </tr>
          </thead>
          <tbody>
            {reservation.map((item, index) => (
              <tr key={index}>
                <td className={dash.td1}>
                  <img src={img} alt="user avatar" />
                </td>
                <td className={dash.td1}>{item.Fullname}</td>
                <td className={dash.td1}>{item.Number}</td>
                <td className={dash.td1}>{item.Cin}</td>
                <td className={dash.td1}>{item.Email}</td>
                <td className={dash.td1}>{item.Event}</td>
                <td className={dash.td1}>{item.NumberPlaces}</td>
                <td className={dash.td1}>
                  <button
