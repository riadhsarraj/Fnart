...
export default function DashEvent2() {
        ...
        try {
            ...
            if (confirmation) {
                const response = await Axios.delete(
                    `http://localhost:3001/DeleteEvent/${id}`
                );
  ...
  return (
    <div className={dash.bod}>
      ...
      <div className="col">
        <h1 className={dash.h1}>List of events</h1>
          ...
          <tbody>
            {Events.map((item, index) => (
              <tr key={index}>
                ...
                <td className={dash.td}>{item.Title}</td>
                <td className={dash.td}>{item.Date}</td>
                <td className={dash.td}>
                  <button
