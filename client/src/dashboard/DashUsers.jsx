...
export default function DashUsers() {
  ...
  return (
    <div className={dash.bod}>
      ...
      <div>
      <h1 className={dash.h1}>Liste des Utilisateurs</h1>
  ...
  <tbody>
    {users.map((item, index) => (
      <tr key={index}>
        <td className={dash.td}><img src={img} alt="user avatar" /></td>
        <td className={dash.td}>{item.username}</td>
        <td className={dash.td}>{item.email}</td>
        <td className={dash.td}>
          <button className={dash.button} onClick={() => Deleteuser(item._id)}>
